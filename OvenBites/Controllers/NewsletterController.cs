        ...
        public async Task<IActionResult> Pay([FromBody] PaymentRequestModel model)
        {
            ...
            try
            {
                ...
                if (calculatedDeliveryCost > 0)
                {
                    ...
                    fulfillmentMessage = "We'll notify you once your order has been shipped. If you have any questions, please reply to this email.";
                }
                else
                {
                    // If the delivery cost is zero, it means the customer chose pickup.
                    fulfillmentMessage = "Your order is ready for pickup. We will send a separate email with pickup instructions. If you have any questions, please reply to this email.";
                }

                // A. Email to the Customer
                var customerEmailBody = $"<h3>Hello {model.CustomerDetails.Name},</h3>" +
                                        ...
                                        "</ul>" +
                                        $"<p>{fulfillmentMessage}</p>" + // Inject the new message here
                ...
                var ownerEmailBody = $"<h3>New Order Received!</h3>" +
                                     ...
                                     $"<p><strong>Name:</strong> {model.CustomerDetails.Name}</p>" +
                                     ...
                                     fulfillmentTypeMessage + // Inject the new message here
                                     "<h4>Order Details:</h4>" +
                                     $"<p><strong>Subtotal:</strong> ${model.Summary.Subtotal:F2}</p>" +
                                     $"<p><strong>Delivery Cost:</strong> ${model.Summary.DeliveryCost:F2}</p>" +
                                     $"<p><strong>Total:</strong> ${model.Summary.Total:F2}</p>" +
                ...
                var ownerEmailSent = await _emailService.SendEmailAsync(
                    memberEmail,
                    $"New Order from {model.CustomerDetails.Name}",
                    ownerEmailBody
                );
